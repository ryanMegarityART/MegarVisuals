...
export const Noise = (
  ...
  const processFrame = (data: Uint8Array) => {
    ...
    const valuesAverage = values.reduce(
      (prev: any, curr: any) => prev + curr / values.length,
      0
    );
    ...
    try {
      ...
      while (len--) {
        ...
        ctx.strokeStyle = `rgb(${params.color.r * (scale / 10)},${
          params.color.g * (scale / 10)
        },${params.color.b * (scale / 10)})`;
